...
void ArrayAverage(int[,] array)
{
    ...
    for(int i = 0; i < array.GetLength(0); i++)
    {
        ...
        tempAvg = tempSum/array.GetLength(1);
        AVG[i] = Math.Round(tempAvg, 1);
        tempSum = 0;
    }
    Console.WriteLine($"The list of all the row averages is such: {string.Join(", ", AVG)}.");
}
